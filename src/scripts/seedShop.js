...
function parseArgs(argv) {
  const out = { contacts: 50, messages: 100, events: 10, optinRate: 0.7, dry: false, clear: false, yes: false };
  for (let i = 2; i < argv.length; i++) {
    const a = argv[i];
    const v = argv[i + 1];
    if (a === '--shop') { out.shop = v; i++; }
    else if (a === '--contacts') { out.contacts = Number(v); i++; }
    else if (a === '--messages') { out.messages = Number(v); i++; }
    else if (a === '--events') { out.events = Number(v); i++; }
    else if (a === '--optinRate') { out.optinRate = Number(v); i++; }
...
const FIRST = ['Maria','Eleni','Giannis','Kostas','Nikos','Dimitris','Eirini','Sofia','Petros','Christos','Anna','Katerina','Vasilis','Giorgos','Theodora'];
const LAST  = ['Papadopoulos','Georgiou','Nikolaou','Pechlivanidis','Ioannou','Karagiannis','Konstantinou','Oikonomou','Papanikolaou','Christodoulou'];

function genBody(kind='automation') {
  const bodies = [
    ...
    const msg = await prisma.message.create({
      data: {
        ...
        metadata: { 
          ...
          ...(status !== 'queued' && { sentAt: new Date(Date.now() - randInt(1, 7) * 3600_000).toISOString() }),
          ...(status === 'delivered' && { deliveredAt: new Date().toISOString() }),
          ...(status === 'failed' && { failedAt: new Date().toISOString(), errorCode: 'SEED_FAIL' })
        }
      }
    });
    created.push(msg);
...
async function main() {
  ...
  if (!args.shop) {
    console.error('Usage: node src/scripts/seedShop.js --shop <shop-domain> [--contacts 50] [--messages 100] [--events 10] [--optinRate 0.7] [--clear] [--yes] [--dry]');
    process.exit(1);
  }
  ...
  console.log(`   Plan: contacts=${args.contacts}, messages=${args.messages}, events=${args.events}, optinRate=${args.optinRate}${args.clear ? ' [CLEAR BEFORE]' : ''}${args.dry ? ' [DRY RUN]' : ''}`);

  if (args.clear && !args.dry && !args.yes) {
    const rl = readline.createInterface({ input, output });
    const ans = await rl.question('This will DELETE contacts/messages/events for this shop. Type "yes" to continue: ');
    rl.close();
  ...
  console.log(`✓ Seed complete: ${contacts.length} contacts, ${messages.length} messages, ${events.length} events`);
}
